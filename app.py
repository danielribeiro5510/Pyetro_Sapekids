...
def new_sale():
    ...
    if request.method == "POST":
        ...
        if action == "finish":
            ...
            try:
                ...
                if conn.postgres:
                    cur = conn.execute(
                        """INSERT INTO sales (total, subtotal, discount_percent, customer_id, payment_method, username, created_at)
                           VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id""",
                        (total, subtotal_total, discount_percent, customer_id, payment_method, username, now)
                    )
    ...
    return render_template("new_sale.html", products=products, customers=customers, cart=cart, total=subtotal, subtotal=subtotal)
