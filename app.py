                ...
                for product, quantity, unit_price, item_subtotal in checked:
                    conn.execute(
                        """INSERT INTO sale_items (sale_id, product_id, quantity, unit_price, subtotal)
                           VALUES (?, ?, ?, ?, ?)""",
                        (sale_id, product["id"], quantity, unit_price, item_subtotal)
                    )
...
def cash():
    ...
    if current:
        cash_sales = conn.execute("""
            ...
            FROM sales WHERE cash_session_id=? GROUP BY payment_method ORDER BY total DESC
        """, (current["id"],)).fetchall()
        cash_total = conn.execute("""
            SELECT COALESCE(SUM(total),0) total FROM sales
            WHERE cash_session_id=? AND payment_method='Dinheiro'
        """, (current["id"],)).fetchone()["total"]
        expected_cash = float(current["opening_amount"] or 0) + float(cash_total or 0)
    else:
        cash_sales=[]; expected_cash=0
    conn.close()
    return render_template("cash.html", current=current, history=history, cash_sales=cash_sales, expected_cash=expected_cash)
...
def admin_backup():
    ...
    tables = ["users","products","movements","sales","sale_items","customers","audit_logs","cash_sessions"]
    mem = io.BytesIO()
